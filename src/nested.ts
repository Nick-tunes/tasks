...
export function sumPoints(questions: Question[]): number {
    const total = questions.reduce((acc, question) => acc + question.points, 0);
    return total;
}
...
export function sumPublishedPoints(questions: Question[]): number {
    const publishedTotal = questions.reduce(
        (total, question) =>
...
export function publishAll(questions: Question[]): Question[] {
    const newQ: Question[] = questions.map((question) =>
...
export function sameType(questions: Question[]): boolean {
    const sameType = questions.every((q) => q.type === questions[0].type);
    return sameType;
}
...
export function addNewQuestion(
    ...
    const newQ: Question[] = [...questions, makeBlankQuestion(id, name, type)];
    return newQ;
}
...
export function renameQuestionById(
    ...
    const targetQ = questions.map((question) =>
...
export function editOption(
    ...
    function editOptions(options: string[]): string[] {
    ...
    const newQ = questions.map((question) =>
        question.id === targetId ?
            { ...question, options: editOptions(question.options) }
        :   question,
    );
    return newQ;
}
